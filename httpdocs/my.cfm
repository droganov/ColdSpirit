{	scopeName = 'text.html.cfm';
	...
	foldingStartMarker = '(?x)
		(<(?i:head|body|table|thead|tbody|tfoot|tr|td|div|select|fieldset|style|script|ul|ol|form|dl|cfloop|cfif|cfswitch|cfcomponent|cffunction|cfquery|cfscript|cfcase|cfsavecontent)\b.*?>
		|<!---(?!.*---\s*>)
		)';
	foldingStopMarker = '(?x)
		(</(?i:head|body|table|thead|tbody|tfoot|tr|td|div|select|fieldset|style|script|ul|ol|form|dl|cfloop|cfif|cfswitch|cfcomponent|cffunction|cfquery|cfscript|cfcase|cfsavecontent)>
		|^(?!.*?<!---).*?---\s*>
		)';
	patterns = (
	...
	repository = {
		cf-string-double-quoted = {
			name = 'string.quoted.double.html.cfm';
			begin = '"';
			end = '"';
		};
		cf-tag-generic-attribute = {
		...
		coldfusion-script = {
			name = 'source.coldfusion.embedded.cfm';
			begin = '#';
			end = '\#';
		...
		entities = {
			patterns = (
				{	name = 'constant.character.entity.cfm';
